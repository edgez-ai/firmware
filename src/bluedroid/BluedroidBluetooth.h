...
// Forward declarations for global helpers already existing in codebase
extern const char *getDeviceName();
extern void put_le32(uint8_t *dst, uint32_t v);
extern PowerFSM powerFSM; // state machine
extern std::shared_ptr<class meshtastic::BluetoothStatus> bluetoothStatus; // status publisher
...
class BluedroidBluetooth : public BluetoothApi
{
  public:
    ...
    int getRssi() override;         // Last read RSSI (actively requested)
